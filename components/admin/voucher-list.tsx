...
import { Eye, Trash2, Copy, Search, Calendar, User, Hotel, Edit } from 'lucide-react';
...
export const VoucherList: React.FC<VoucherListProps> = ({ vouchers }) => {
  ...
  const filtered = useMemo(
    () =>
      vouchers.filter((v) =>
        [v.travelId, v.clientName, v.itinerary.clientPhone, v.itinerary.packageTitle].some((val) =>
                ...
                <TableCell colSpan={7} className="text-center py-12">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-gray-500 dark:text-gray-400">
                      {search ? 'No vouchers found matching your search' : 'No vouchers found'}
                    </p>
                  </div>
                </TableCell>
              </TableRow>
              ...
              <TableRow key={voucher.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                <TableCell>
                  <div className="flex items-center gap-2">
                    <span className="font-mono font-bold text-purple-600 dark:text-purple-400">{voucher.travelId}</span>
                    <Button
                      size="icon"
                      variant="ghost"
                      onClick={() => copyTravelId(voucher.travelId)}
                ...
                <TableCell>
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">{voucher.itinerary.clientPhone}</p>
                    {voucher.itinerary.clientEmail && (
                ...
                <TableCell className="max-w-xs">
                  <p className="font-medium truncate" title={voucher.itinerary.packageTitle}>
                    {voucher.itinerary.packageTitle}
                  </p>
                </TableCell>
                <TableCell>
                  <div className="space-y-1">
                ...
                <TableCell>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-semibold">{voucher.totalNights}N</span>
                  </div>
                </TableCell>
                <TableCell>
                  <p className="text-sm text-gray-600 dark:text-gray-400">{formatDate(voucher.createdAt)}</p>
                </TableCell>
                <TableCell>
                  <div className="flex gap-2 justify-end">
                    ...
                    <Link href={`/voucher/view/${voucher.travelId}`} target="_blank">
                      <Button
                        size="sm"
                        variant="ghost"
                        title="Open Voucher"
